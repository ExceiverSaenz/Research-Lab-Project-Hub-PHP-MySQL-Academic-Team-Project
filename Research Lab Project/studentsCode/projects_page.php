...
$query = "SELECT DISTINCT P.PID, P.PName, P.PObjective, P.PStartDate, P.PEndDate
          FROM project P
           ...
           <div class="card-body">
               ...
               if (isset($_GET['PID'])) {
                $report_sql = "SELECT p.*, 
                            ...
                            WHERE p.PID IN (SELECT ProjectID FROM PARTICIPATE WHERE StudentID = ?) 
                            GROUP BY p.PID";
                   
                   $stmt = $conn->prepare($report_sql);
                   $stmt->bind_param("ii", $_GET['PID'], $_SESSION['StudentID']);
                   $stmt->execute();
